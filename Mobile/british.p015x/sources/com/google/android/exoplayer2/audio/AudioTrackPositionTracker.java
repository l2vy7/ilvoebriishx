...
final class AudioTrackPositionTracker {
    ...
    private long getPlaybackHeadPosition() {
        ...
        if (this.stopTimestampUs != C6540C.TIME_UNSET) {
        ...
        if (Util.SDK_INT <= 29) {
            if (playbackHeadPosition == 0 && this.lastRawPlaybackHeadPosition > 0 && playState == 3) {
                if (this.forceResetWorkaroundTimeMs == C6540C.TIME_UNSET) {
                    this.forceResetWorkaroundTimeMs = SystemClock.elapsedRealtime();
                }
                return this.lastRawPlaybackHeadPosition;
            }
            this.forceResetWorkaroundTimeMs = C6540C.TIME_UNSET;
        }
    ...
    public long getCurrentPositionUs(boolean z) {
        ...
        if (!this.notifiedPositionIncreasing) {
            ...
            if (j > j4) {
                ...
                this.listener.onPositionAdvancing(System.currentTimeMillis() - C6540C.usToMs(Util.getPlayoutDurationForMediaDuration(C6540C.usToMs(j - j4), this.audioTrackPlaybackSpeed)));
            }
        }
    ...
    public long getPendingBufferDurationMs(long j) {
        return C6540C.usToMs(framesToDurationUs(j - getPlaybackHeadPosition()));
    }
    ...
    public boolean isStalled(long j) {
        return this.forceResetWorkaroundTimeMs != C6540C.TIME_UNSET && j > 0 && SystemClock.elapsedRealtime() - this.forceResetWorkaroundTimeMs >= FORCE_RESET_WORKAROUND_TIMEOUT_MS;
    }
    ...
    public boolean mayHandleBuffer(long j) {
        ...
        if (z && !hasPendingData && playState != 1) {
            this.listener.onUnderrun(this.bufferSize, C6540C.usToMs(this.bufferSizeUs));
        }
        return true;
    }
    ...
    public void setAudioTrack(AudioTrack audioTrack2, boolean z, int i, int i2, int i3) {
        ...
        this.hasData = false;
        this.stopTimestampUs = C6540C.TIME_UNSET;
        this.forceResetWorkaroundTimeMs = C6540C.TIME_UNSET;
