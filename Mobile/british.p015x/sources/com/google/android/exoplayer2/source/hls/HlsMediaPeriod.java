...
public final class HlsMediaPeriod implements MediaPeriod, HlsSampleStreamWrapper.Callback, HlsPlaylistTracker.PlaylistEventListener {
    ...
    private void buildAndPrepareAudioSampleStreamWrappers(long j, List<HlsMasterPlaylist.Rendition> list, List<HlsSampleStreamWrapper> list2, List<int[]> list3, Map<String, DrmInitData> map) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (!hashSet.add(str)) {
                ...
                list3.add(C11054c.m49465i(arrayList3));
    ...
    private static Format deriveAudioFormat(Format format, Format format2, boolean z) {
        ...
        return new Format.Builder().setId(format.f27194id).setLabel(str).setContainerMimeType(format.containerMimeType).setSampleMimeType(mediaMimeType).setCodecs(str3).setMetadata(metadata).setAverageBitrate(i12).setPeakBitrate(i4).setChannelCount(i).setSelectionFlags(i2).setRoleFlags(i3).setLanguage(str2).build();
    }
