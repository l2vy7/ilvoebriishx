package com.google.ads.interactivemedia.p039v3.internal;

import java.security.GeneralSecurityException;

/* renamed from: com.google.ads.interactivemedia.v3.internal.ayc */
/* compiled from: IMASDK */
final class ayc extends axb<bar, baq> {

    /* renamed from: a */
    final /* synthetic */ ayd f15902a;

    /* JADX INFO: super call moved to the top of the method (can break code semantics) */
    ayc(ayd ayd, Class cls) {
        super(cls);
        this.f15902a = ayd;
    }

    /* renamed from: b */
    public final /* bridge */ /* synthetic */ void mo14729b(bln bln) throws GeneralSecurityException {
        bar bar = (bar) bln;
        bea.m15756a(bar.mo14809c());
        ayd.m15191l(bar.mo14808a());
    }

    /* renamed from: c */
    public final /* bridge */ /* synthetic */ bln mo14730c(bjq bjq) throws bkt {
        return bar.m15405d(bjq, bka.m16275a());
    }

    /* renamed from: d */
    public final /* bridge */ /* synthetic */ Object mo14731d(bln bln) throws GeneralSecurityException {
        bar bar = (bar) bln;
        bap f = baq.m15395f();
        f.mo14803b(bar.mo14808a());
        f.mo14802a(bjq.m16051n(bdz.m15753a(bar.mo14809c())));
        f.mo14804c();
        return (baq) f.mo15385ae();
    }
}
