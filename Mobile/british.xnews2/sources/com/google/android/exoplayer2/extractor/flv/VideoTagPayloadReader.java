...
final class VideoTagPayloadReader extends TagPayloadReader {
    ...
    public boolean parsePayload(ParsableByteArray parsableByteArray, long j) throws ParserException {
        ...
        if (readUnsignedByte == 0 && !this.hasOutputFormat) {
            ...
            this.output.format(new Format.Builder().setSampleMimeType(MimeTypes.VIDEO_H264).setCodecs(parse.codecs).setWidth(parse.width).setHeight(parse.height).setPixelWidthHeightRatio(parse.pixelWidthAspectRatio).setInitializationData(parse.initializationData).build());
