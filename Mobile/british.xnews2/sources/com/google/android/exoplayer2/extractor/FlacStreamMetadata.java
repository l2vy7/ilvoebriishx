...
public final class FlacStreamMetadata {
    ...
    private static Metadata buildMetadata(List<String> list, List<PictureFrame> list2) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (splitAtFirst.length != 2) {
                ...
                Log.m19797w(TAG, valueOf.length() != 0 ? "Failed to parse Vorbis comment: ".concat(valueOf) : new String("Failed to parse Vorbis comment: "));
            } else {
    ...
    public long getDurationUs() {
        ...
        return j == 0 ? C6540C.TIME_UNSET : (j * 1000000) / ((long) this.sampleRate);
    }
