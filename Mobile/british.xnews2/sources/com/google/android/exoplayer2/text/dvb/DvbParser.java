...
final class DvbParser {
    ...
    private static void parseSubtitlingSegment(ParsableBitArray parsableBitArray, SubtitleService subtitleService2) {
        ...
        if (readBits3 * 8 > parsableBitArray.bitsLeft()) {
            Log.m19797w(TAG, "Data field length exceeds limit");
        ...
        switch (readBits) {
            ...
            case 17:
                ...
                if (readBits2 == subtitleService2.subtitlePageId && pageComposition2 != null) {
                    ...
                    if (pageComposition2.state == 0 && (regionComposition = subtitleService2.regions.get(parseRegionComposition.f27607id)) != null) {
                        parseRegionComposition.mergeFrom(regionComposition);
                    }
                    subtitleService2.regions.put(parseRegionComposition.f27607id, parseRegionComposition);
                    break;
                }
            case 18:
                if (readBits2 != subtitleService2.subtitlePageId) {
                    if (readBits2 == subtitleService2.ancillaryPageId) {
                        ...
                        subtitleService2.ancillaryCluts.put(parseClutDefinition.f27605id, parseClutDefinition);
                        break;
                    }
                } else {
                    ClutDefinition parseClutDefinition2 = parseClutDefinition(parsableBitArray, readBits3);
                    subtitleService2.cluts.put(parseClutDefinition2.f27605id, parseClutDefinition2);
                    break;
                }
                break;
            case 19:
                if (readBits2 != subtitleService2.subtitlePageId) {
                    if (readBits2 == subtitleService2.ancillaryPageId) {
                        ...
                        subtitleService2.ancillaryObjects.put(parseObjectData.f27606id, parseObjectData);
                        break;
                    }
                } else {
                    ObjectData parseObjectData2 = parseObjectData(parsableBitArray);
                    subtitleService2.objects.put(parseObjectData2.f27606id, parseObjectData2);
                    break;
                }
                break;
            case 20:
        ...
        for (int i4 = 0; i4 < sparseArray2.size(); i4++) {
            ...
            while (i7 < sparseArray3.size()) {
                ...
                if (objectData2 != null) {
                    ...
                    i3 = i7;
                    int i8 = i6 + valueAt2.verticalPosition;
                    sparseArray = sparseArray3;
                    paintPixelDataSubBlocks(objectData2, clutDefinition, regionComposition.depth, valueAt2.horizontalPosition + i5, i8, paint, this.canvas);
                } else {
                    i3 = i7;
                    sparseArray = sparseArray3;
                }
