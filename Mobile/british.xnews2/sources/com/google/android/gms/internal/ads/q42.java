package com.google.android.gms.internal.ads;

import android.content.Context;
import android.view.View;
import android.webkit.WebView;
import p198d5.C10487a;

/* compiled from: com.google.android.gms:play-services-ads@@20.6.0 */
public final class q42 implements sg0 {

    /* renamed from: a */
    private final sg0 f37857a;

    /* renamed from: b */
    private final sg0 f37858b;

    public q42(sg0 sg0, sg0 sg02) {
        this.f37857a = sg0;
        this.f37858b = sg02;
    }

    /* renamed from: a */
    private final sg0 m36286a() {
        return ((Boolean) C8311wv.m39277c().mo18570b(p00.f36859D3)).booleanValue() ? this.f37857a : this.f37858b;
    }

    /* renamed from: b */
    public final String mo34045b(Context context) {
        return m36286a().mo34045b(context);
    }

    /* renamed from: c */
    public final void mo34046c(C10487a aVar, View view) {
        m36286a().mo34046c(aVar, view);
    }

    /* renamed from: d */
    public final C10487a mo34047d(String str, WebView webView, String str2, String str3, String str4, ug0 ug0, tg0 tg0, String str5) {
        return m36286a().mo34047d(str, webView, "", "javascript", str4, ug0, tg0, str5);
    }

    /* renamed from: e */
    public final C10487a mo34048e(String str, WebView webView, String str2, String str3, String str4, String str5, ug0 ug0, tg0 tg0, String str6) {
        return m36286a().mo34048e(str, webView, "", "javascript", str4, str5, ug0, tg0, str6);
    }

    /* renamed from: f */
    public final boolean mo34049f(Context context) {
        return m36286a().mo34049f(context);
    }

    /* renamed from: g */
    public final void mo34050g(C10487a aVar, View view) {
        m36286a().mo34050g(aVar, view);
    }

    public final void zze(C10487a aVar) {
        m36286a().zze(aVar);
    }

    public final void zzh(C10487a aVar) {
        m36286a().zzh(aVar);
    }
}
