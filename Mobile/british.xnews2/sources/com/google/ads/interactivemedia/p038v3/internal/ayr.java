package com.google.ads.interactivemedia.p038v3.internal;

import java.security.GeneralSecurityException;

/* renamed from: com.google.ads.interactivemedia.v3.internal.ayr */
/* compiled from: IMASDK */
final class ayr extends axb<bcm, bcl> {

    /* renamed from: a */
    final /* synthetic */ ays f15907a;

    /* JADX INFO: super call moved to the top of the method (can break code semantics) */
    ayr(ays ays, Class cls) {
        super(cls);
        this.f15907a = ays;
    }

    /* renamed from: b */
    public final /* bridge */ /* synthetic */ void mo14729b(bln bln) throws GeneralSecurityException {
        bcm bcm = (bcm) bln;
    }

    /* renamed from: c */
    public final /* bridge */ /* synthetic */ bln mo14730c(bjq bjq) throws bkt {
        return bcm.m15649c(bjq, bka.m16275a());
    }

    /* renamed from: d */
    public final /* bridge */ /* synthetic */ Object mo14731d(bln bln) throws GeneralSecurityException {
        bck e = bcl.m15642e();
        e.mo14913a((bcm) bln);
        e.mo14914b();
        return (bcl) e.mo15385ae();
    }
}
